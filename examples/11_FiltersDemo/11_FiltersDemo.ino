...
#include "experimental/evaAdaptiveSmoothReader.h"
#include "experimental/evaMedianReader.h"
#include "experimental/evaMinmaxReader.h"
#include "experimental/evaSlidingWindowReader.h"
#include "experimental/evaExponentialSmoothReader.h"
#include "experimental/evaSlewRateReader.h"
...
public:
    SignalGenerator() : Heartbeat(10) {} // Обновление каждые 10 мс
...
// ============================================================================
// Цепи фильтрации (Reader-декораторы над GlobalVarReader)
// ============================================================================

// 1. Адаптивный сглаживающий фильтр (10ms - 150ms)
AdaptiveSmoothReader<GlobalVarReader, 10, 150> adaptiveSmoothFilter;

// 2. Медианный фильтр (окно = 5 элементов)
MedianReader<GlobalVarReader, 3> medianFilter;

// 3. Морфологический MinMax фильтр (N = 3, буфер N*N = 9)
MinmaxReader<GlobalVarReader, 2> minmaxFilter;

// 4. Скользящее среднее (окно = 8 элементов)
SlidingWindowReader<GlobalVarReader, 3> slidingWindowFilter;

// 5. Экспоненциальный фильтр EMA (alpha = 150 / 1000)
ExponentialSmoothReader<GlobalVarReader, 500> exponentialSmoothFilter;

// 6. Ограничитель скорости изменения (макс. шаг 30 ед. за тик 10ms)
SlewRateReader<GlobalVarReader, 30> slewRateFilter;
